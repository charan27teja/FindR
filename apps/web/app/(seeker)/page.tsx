...
export default async function Home() {
  ...
  const [{ data: orgs }, { data: memberships }, { data: events }, notices] = await Promise.all([
    ...
    supabase
      ...
      .order("event_date")
      .limit(200),
    // Claim notices for the bell — no dependency on the queries above, so they
    // ride along in the same Promise.all instead of waiting for them to finish.
    fetchClaimNotices(),
  ]);
